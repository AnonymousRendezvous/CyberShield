...
def payload_gen(word, add, img):
    ...
    if img == "y":
        ...
        payload2 += " Do remember to include ALL links and make verbose inferences to the best of your ability! Here is some starting data for you. Data: " + gptstring
        payload3 += " Here is a final data check on the report. Clean up your report and remember to KEEP ALL content from your previous response(s) and ADD ON to everything especially the links at the bottom (numbered), being as VERBOSE as possible! Data: " + gptstring2 + gptstring3
        payloads.append(payload)
        payloads.append(payload2)
        for x in range(len(http)):
                    key_content = extract_text_from_website(http[x])
                    links = "Now for each of the links you have identified, I will give you the content for each of them. Remember to ADD ON to your previous response, being as verbose as possible. Link: " + http[x] + ' ' + key_content + ". "
